...
import {
  ...
  SYFOOPPFOLGINGSPLANBACKEND_ROOT,
...
export function useGetOppfolgingsplanerV2Query() {
  const fnr = useValgtPersonident();
  const path = `${SYFOOPPFOLGINGSPLANBACKEND_ROOT}/oppfolgingsplaner/query`;
  const payload: OppfolgingsplanV2RequestBody = {
    sykmeldtFnr: fnr,
  };
  const fetchNewOppfolgingsplaner = () =>
    post<OppfolgingsplanV2DTO[]>(path, payload, fnr);
  const query = useQuery({
    queryKey: oppfolgingsplanQueryKeys.oppfolgingsplanerV2(fnr),
    queryFn: fetchNewOppfolgingsplaner,
    enabled: !!fnr,
    staleTime: minutesToMillis(60 * 12),
  });
