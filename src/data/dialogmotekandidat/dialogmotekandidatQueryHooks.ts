...
const useAvventQuery = () => {
  ...
  const fetchAvvent = () => get<AvventDTO | null>(path, personident);
  ...
  const avventDto = query.data;
  const avvent =
    avventDto &&
    ({
      frist: avventDto.frist ? new Date(avventDto.frist) : undefined,
      tekst: avventDto.beskrivelse ?? "",
    } as const);
