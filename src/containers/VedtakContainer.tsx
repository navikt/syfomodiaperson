...
const groupVedtakByOrgnr = (
  vedtak: VedtakSuperContainer[]
): VedtakSuperContainer[][] => {
  const orgMap = vedtak.reduce(
    (
      acc: Map<String, VedtakSuperContainer[]>,
      currentValue: VedtakSuperContainer
    ) => {
    ...
    new Map<String, VedtakSuperContainer[]>()
  );
...
const VedtakContainer = () => {
  ...
  const [selectedVedtak, setSelectedVedtak] = useState<VedtakSuperContainer>();
    ...
    <Side fnr={fnr} tittel={texts.pageTitle} aktivtMenypunkt={VEDTAK}>
      <>
        ...
        {vedtakState && selectedVedtak && (
          <Row>
            <Column className="col-xs-5 vedtak">
              {groupVedtakByOrgnr(vedtakState.data).map(
                (vedtakPerArbeidsgiver: VedtakSuperContainer[]) => (
                  <VedtakEkspanderbartPanel
                    ...
                    setSelectedVedtak={(x: VedtakSuperContainer) => {
                      setSelectedVedtak(x);
                    }}
