...
export function PersonkortHeaderTags() {
  ...
  const mostRecentValidSykmelding = sykmeldingerSortertNyestTilEldstPeriode(
    getSykmeldingerQuery.sykmeldinger
  ).find(
    (sykmelding) =>
      sykmelding.status !== SykmeldingStatus.NY &&
      sykmelding.status !== SykmeldingStatus.AVBRUTT &&
      sykmelding.status !== SykmeldingStatus.UTGAATT
  );
  const harIkkeArbeidsgiver = mostRecentValidSykmelding
    ? erSykmeldingUtenArbeidsgiver(mostRecentValidSykmelding)
    : false;
